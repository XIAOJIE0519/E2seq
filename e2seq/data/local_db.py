"""Local database access for E2seq."""
...
def initialize_databases(data_dir: Path) -> None:
    ...
    for db_name, (csv_file, table_name) in databases.items():
        ...
        try:
            ...
            # These lookups run once per selected expression item during a
            # bulk RAG handoff.  Without indexes SQLite scans the full local
            # table for every gene (STRING is ~1.8M rows), which needlessly
            # drives CPU and makes a large batch appear stuck.
            index_sql = {
                "string": (
                    'CREATE INDEX IF NOT EXISTS idx_string_source_gene '
                    'ON string_interactions(source_gene)',
                    'CREATE INDEX IF NOT EXISTS idx_string_target_gene '
                    'ON string_interactions(target_gene)',
                ),
                "hmdb": (
                    'CREATE INDEX IF NOT EXISTS idx_hmdb_gene_name '
                    'ON hmdb_associations(gene_name)',
                ),
                "trrust": (
                    'CREATE INDEX IF NOT EXISTS idx_trrust_tf '
                    'ON trrust_regulations("TF")',
                    'CREATE INDEX IF NOT EXISTS idx_trrust_gene '
                    'ON trrust_regulations("gene")',
                ),
                "gutmgene": (
                    'CREATE INDEX IF NOT EXISTS idx_gutmgene_gene '
                    'ON gutmgene_associations("Gene")',
                    'CREATE INDEX IF NOT EXISTS idx_gutmgene_gene_upper '
                    'ON gutmgene_associations(UPPER("Gene"))',
                ),
            }
            for statement in index_sql.get(db_name, ()):
