"""Memory system for E2seq Agent - Short-term and Long-term memory."""
...
class MemoryManager:
    ...
    def get_conversation_history_for_llm(
        ...
        """Format the working-memory conversation history as a list of
        ``{"role": ..., "content": ...}`` dicts ready to be passed directly
        into a chat-completion ``messages`` payload.

        Older agent call sites that imported this name were written against
        a richer memory manager; the base implementation only keeps
        ``working_memory``, so we map it here.

        Args:
            max_messages: If set, return only the most recent N messages.
            max_total_chars: If set, truncate the trailing messages so the
                total string length stays under this budget.
        """
        try:
            history = self.working_memory.conversation_history or []
        except Exception:
            history = []
        ...
        if max_total_chars and max_total_chars > 0:
            ...
            for msg in reversed(out):
                ...
                if running + content_len > max_total_chars and trimmed:
                    continue
                running += content_len
                trimmed.append(msg)
...
def delete_session_memory(session_id: str) -> Dict[str, Any]:
    """Remove long-term memory records created by one chat session.

    Long-term memory is shared for retrieval, so leaving a deleted chat's
    summary in ``sessions.jsonl`` or ``knowledge.json`` would allow it to
    reappear in a later answer.  Only records carrying the exact session ID
    are removed; learned patterns and other chats remain untouched.
    """
    key = str(session_id or "default")
    result: Dict[str, Any] = {"session_records": 0, "knowledge_records": 0, "vector_cases": 0}
    try:
        memory_dir = Path(get_config().database.db_path).expanduser() / "memory"
        session_file = memory_dir / "sessions.jsonl"
        if session_file.is_file():
            ...
            with session_file.open("r", encoding="utf-8") as handle:
                for line in handle:
                    try:
                        ...
                        continue
                    if isinstance(item, dict) and str(item.get("session_id") or "") == key:
            ...
            try:
                with tempfile.NamedTemporaryFile(
                    mode="w", encoding="utf-8", dir=str(memory_dir),
                    prefix=".sessions.", suffix=".tmp", delete=False,
                ) as handle:
        ...
        if knowledge_file.is_file():
            ...
            if isinstance(payload, dict):
                for bucket in ("successful_analyses", "failed_analyses"):
        ...
        # MemoryManager stores cases in the shared default collection.  Delete
        # only this chat's case ID; per-chat RAG collections are handled by
        # delete_vector_store() in the API cleanup path.
        try:
            from e2seq.data.vector_store import delete_vector_store_cases
            result["vector_cases"] = delete_vector_store_cases([key], session_id="default")
        except Exception as exc:
            logger.debug("Failed to remove long-term vector case for %s: %s", key, exc)
    except OSError as exc:
        logger.warning("Failed to delete long-term memory for %s: %s", key, exc)
    return result
