...
class EnhancedPlannerAgent:
    ...
    def create_plan(self, question: str, dataset_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Get plan from LLM
        messages = [
            {"role": "system", "content": "You are an expert expression-profile and single-cell analysis planner."},
    ...
    def _parse_steps(self, question: str) -> List[Dict[str, Any]]:
        ...
        if any(kw in q_lower for kw in ["宸紓", "differential", "deg"]):
            steps.append({"action": "differential_expression", "params": {}})

        if any(kw in q_lower for kw in ["瀵岄泦", "enrichment", "go", "kegg"]):
            steps.append({"action": "enrichment_analysis", "params": {}})

        if any(kw in q_lower for kw in ["缃戠粶", "network", "ppi"]):
