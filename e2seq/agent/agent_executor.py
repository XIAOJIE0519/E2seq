"""LangGraph-based Agent Executor for E2seq.
...
from e2seq.agent.tool_registry import ToolRegistry
...
## Rules
1. Answer the user's actual request directly and follow the requested language, format, scope, and detail.
2. Use tools only when they help answer that request; do not add an unrelated fixed workflow.
3. Never invent data or completed computations; distinguish uploaded results from external annotations.
4. Cite the tool/database when it supports a biological claim and state uncertainty when evidence is limited.
"""


class E2seqAgentExecutor:
...
def create_agent_executor(llm, tool_registry: ToolRegistry) -> E2seqAgentExecutor:
    """Factory function."""
    return E2seqAgentExecutor(llm, tool_registry)
