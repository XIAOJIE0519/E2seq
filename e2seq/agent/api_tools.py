...
def register_api_tools(registry) -> None:
    ...
    # 10. GTEx - 基因组织表达
    # The legacy GTEx wrapper mixed HPA/NCBI fallbacks into a GTEx label.
    # The verified source adapter registered below keeps the source identity
    # honest and queries the current GTEx Portal v2 endpoint.
    gtex = None
    if gtex:
    ...
    # HumanBase's former public endpoint is not a stable query service.  Do
    # not register the old HPA/Jensen fallback under the HumanBase name.
    humanbase = None
    if humanbase:
    ...
    # Alliance is registered through the maintained /api/search adapter below.
    alliance = None
    if alliance:
    ...
    # Verified source adapters.  Keep these tools explicit so the agent can
    # name the concrete endpoint instead of hiding everything behind Graph RAG.
    try:
        from e2seq.data.knowledge_sources import KnowledgeSourceClient
        verified_client = KnowledgeSourceClient(timeout=20)
        verified_sources = (
        ...
        for source in verified_sources:
            registry.register_tool(
                f"{source}_query",
                lambda gene, max_results=20, _source=source: str(
                    verified_client.query(_source, gene, max_results=max_results)
                ),
                f"Query the verified {source} source for one human gene and return status, records, and coverage.",
                {
                    "type": "object",
                    "properties": {
                        "gene": {"type": "string"},
        ...
        logger.warning(f"Verified source tools unavailable: {exc}")

    new_prefixes = ('gtex_', 'hpa_', 'humanbase_', 'gwas_', 'civic_', 'alliance_', 'cbioportal_', 'omnipath_', 'intact_')
