"""State management for E2seq Agent - Persistent state tracking."""
...
class StateManager:
    ...
    def __init__(self, session_id: Optional[str] = None):
        ...
        # The old implementation used one global state file for every chat.
        # A completed task in one conversation could therefore be loaded as
        # the initial state of another conversation or test.  Keep the legacy
        # default file for callers that explicitly use the global manager,
        # while giving every real chat session an isolated state namespace.
        if session_id is None:
            self.state_file = self.state_dir / "current_state.json"
            self.checkpoint_dir = self.state_dir / "checkpoints"
        else:
            scoped_dir = self.state_dir / "sessions"
            scoped_dir.mkdir(parents=True, exist_ok=True)
            safe_id = _safe_session_id(self.session_id)
            self.state_file = scoped_dir / f"{safe_id}_current_state.json"
            self.checkpoint_dir = scoped_dir / safe_id / "checkpoints"
...
def get_state_manager(session_id: Optional[str] = None) -> StateManager:
    ...
    if "_state_managers" not in globals():
        _state_managers = {}
    key = str(session_id or "default")
    if key not in _state_managers:
        _state_managers[key] = StateManager(session_id=session_id)
    return _state_managers[key]


def delete_session_state(session_id: str) -> List[str]:
    """Remove one chat's cached state and checkpoints from disk and memory."""
    key = str(session_id or "default")
    deleted: List[str] = []

    managers = globals().get("_state_managers", {})
    if isinstance(managers, dict) and managers.pop(key, None) is not None:
        deleted.append(f"memory:{key}")

    try:
        state_root = Path(get_config().database.db_path).expanduser() / "state" / "sessions"
        safe_id = _safe_session_id(key)
        state_file = state_root / f"{safe_id}_current_state.json"
        checkpoint_dir = state_root / safe_id / "checkpoints"

        if state_file.is_file():
            state_file.unlink()
            deleted.append(f"file:{state_file.name}")
        if checkpoint_dir.is_dir():
            shutil.rmtree(checkpoint_dir)
            deleted.append(f"dir:{checkpoint_dir}")

        # Remove the now-empty per-session directory, but never remove the
        # shared ``sessions`` parent or any other session directory.
        session_dir = state_root / safe_id
        if session_dir.is_dir() and not any(session_dir.iterdir()):
            session_dir.rmdir()
    except OSError as exc:
        logger.warning("Failed to delete persisted state for %s: %s", key, exc)
