"""Agent orchestrator for E2seq with streaming support."""

from typing import Any, Dict, Generator, List, Optional

from anndata import AnnData

from e2seq.agent.enhanced_planner import EnhancedPlannerAgent
from e2seq.agent.memory import get_memory_manager
from e2seq.agent.planner import PlannerAgent
from e2seq.agent.retriever import RetrieverAgent
from e2seq.agent.synthesizer import SynthesizerAgent
from e2seq.data.api_client import create_api_clients
from e2seq.data.local_db import GUTMGENEDatabase, HMDBDatabase, STRINGDatabase, TRRUSTDatabase
from e2seq.llm import create_llm_provider
from e2seq.tools import EnrichmentAnalyzer, NetworkAnalyzer, ScancpyTools, Visualizer
...
class E2seqAgent:
    """Main agent orchestrator for E2seq with streaming support."""
    ...
    def __init__(
        ...
        """Initialize E2seq agent."""
        logger.info("Initializing E2seq Agent")
        ...
        logger.info("E2seq Agent initialized successfully")
        ...
        if self.adata is None:
            ...
            try:
                if hasattr(self, 'retriever') and self.retriever:
                    # RetrieverAgent expects the second argument to be the
                    # analysis-results dictionary.  Passing ``dataset_info``
                    # here was an old call-site mismatch and made no-data
                    # chat return a mock/error instead of a normal reply.
                    db_results = self.retriever.retrieve(message, {})
                    if isinstance(db_results, dict):
                        db_results = db_results.get("similar_cases", [])
                    if db_results:
            ...
            system_prompt = (
                "You are E2seq, a scientific assistant for user-provided expression and sequencing data. "
                "Answer the user's actual question directly and follow the requested language, format, "
                "scope, and level of detail. Use the available local context only when it helps answer the "
                "question; never invent data or completed computations. Currently no expression dataset is loaded."
            )
            if rag_context:
            ...
            if not isinstance(reply, str):
                reply = "Please load an expression-profile dataset before requesting a data analysis."
            response = {
                "text": reply,
                "plots": [],
                "data": {},
