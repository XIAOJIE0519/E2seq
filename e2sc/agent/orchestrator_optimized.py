...
class E2scAgentOptimized:
    ...
    def _chat_comprehensive(self, message: str, thinking_steps: list) -> dict:
        ...
        # Cap at a reasonable maximum for this fallback path (user can still configure up to 100 in the modern paths)
        n_top = min(n_top, 50)
        output_mode = str(self.adata.uns.get("e2sc_output_mode", "detailed")) if self.adata is not None else "detailed"

        cached = self.memory.working_memory.current_context.get("gene_matrix")
        if cached is None:
            # Use user-configured cell-type column (stored in adata.uns after configure-dataset)
            ct_col = self.adata.uns.get("e2sc_celltype_col", None)
            cached = self.scanpy_tools.get_top_genes_matrix(n_top_genes=n_top, celltype_col=ct_col)
            self.memory.working_memory.update_context("gene_matrix", cached)

        group_cached = self.memory.working_memory.current_context.get("group_matrix")
        if group_cached is None:
            grp_col = self.adata.uns.get("e2sc_group_col", "group")
            group_cached = self.scanpy_tools.get_top_genes_by_group(group_col=grp_col, n_top_genes=n_top)
            self.memory.working_memory.update_context("group_matrix", group_cached)

        # Read user-selected API/DB configuration
        enabled_apis = set(self.adata.uns.get("e2sc_enabled_apis",
            ["uniprot","mygene","quickgo","ensembl","chembl","pubmed","europepmc","reactome","gtex","humanbase","gwas","biogrid","civic","alliance","opentargets","clinvar"]))
        enabled_dbs = set(self.adata.uns.get("e2sc_enabled_dbs",
            ["string","hmdb","trrust","gutmgene"]))
        # User-defined display label mappings (orig -> display)
        group_labels = dict(self.adata.uns.get("e2sc_group_labels", {}))
        celltype_labels = dict(self.adata.uns.get("e2sc_celltype_labels", {}))

        ct_map_raw = cached.get("top_genes_per_celltype", {})
        grp_map_raw = group_cached.get("top_genes_per_group", {})
        # Apply display label mappings: keys become display names, genes unchanged
        ct_map = {celltype_labels.get(k, k): v for k, v in ct_map_raw.items()}
        grp_map = {group_labels.get(k, k): v for k, v in grp_map_raw.items()}

        from collections import Counter
        gene_freq = Counter()
        for genes in ct_map.values():
            gene_freq.update(genes)
        overall_top = [g for g, _ in gene_freq.most_common(n_top)]

        thinking_steps.append({"step": "Matrix", "content": f"{len(ct_map)} cell types, {len(grp_map)} disease groups"})

        # ------------------------------------------------------------------ #
        # Knowledge cache: skip all API queries if results already exist in
        # working memory for this session + dataset combination.
        # ------------------------------------------------------------------ #
        _cache_key = "comprehensive_knowledge"
        _cached_know = self.memory.working_memory.current_context.get(_cache_key)
        if _cached_know is not None:
            ...
            ct_knowledge = {}
            ct_total = len(ct_map)
            for ct_idx, (ct, genes) in enumerate(ct_map.items(), 1):
                logger.info(f"[进度] 细胞类型知识查询 {ct_idx}/{ct_total}: {ct}")
                ct_knowledge[ct] = self._build_group_knowledge(ct, genes, context_hint=ct, enabled_apis=enabled_apis, enabled_dbs=enabled_dbs)
                kg = ct_knowledge[ct]
                thinking_steps.append({"step": f"KB:{ct}", "content": f"{len(kg['genes'])} annotated, {len(kg['pubmed'])} articles"})

            grp_knowledge = {}
            grp_total = len(grp_map)
            for grp_idx, (grp, genes) in enumerate(grp_map.items(), 1):
                logger.info(f"[进度] 疾病表型知识查询 {grp_idx}/{grp_total}: {grp}")
                grp_knowledge[grp] = self._build_group_knowledge(grp, genes, context_hint=f"{grp} inflammatory bowel disease", enabled_apis=enabled_apis, enabled_dbs=enabled_dbs)
                kg = grp_knowledge[grp]
                thinking_steps.append({"step": f"KB:{grp}", "content": f"{len(kg['genes'])} annotated, {len(kg['pubmed'])} articles"})

            # Persist to working memory so subsequent messages reuse this cache
            self.memory.working_memory.update_context(_cache_key, {
                "ct_knowledge":  ct_knowledge,
                "grp_knowledge": grp_knowledge,
            })
            logger.info("[进度] API查询结果已缓存到working memory")

        # Build context string
        lines = []
        lines.append(f"数据集: {self.adata.n_obs}个细胞, {self.adata.n_vars}个基因")
        lines.append(f"细胞类型{len(ct_map)}种, 疾病表型: {list(grp_map.keys())}")

        lines.append("\n=== 各细胞类型 Top15 高表达基因 ===")
        for ct, genes in ct_map.items():
            kb = ct_knowledge.get(ct, {"genes": {}, "pubmed": [], "europepmc": []})
            lines.append(f"\n[{ct}] Top15: {'、'.join(genes)}")
            for g in genes[:8]:
                info = kb["genes"].get(g, {})
                parts = []
                if info.get("function"): parts.append(f"功能 [UniProt]:{info['function'][:120]}")
                if info.get("gene_name"): parts.append(f"官方名 [MyGene]:{info['gene_name']}")
                if info.get("go_terms"): parts.append(f"GO [QuickGO]:{'、'.join(info['go_terms'][:3])}")
                if info.get("gtex_tissues"): parts.append(f"组织表达 [GTEx]:{'、'.join(str(t) for t in info['gtex_tissues'][:3])}")
                if info.get("humanbase_tissues"): parts.append(f"组织特异性 [HumanBase]:{'、'.join(str(t) for t in info['humanbase_tissues'][:3])}")
                if info.get("gwas_snps"): parts.append(f"GWAS [GWAS Catalog]:{'、'.join(str(s) for s in info['gwas_snps'][:2])}")
                if info.get("biogrid_interactions"): parts.append(f"实验互作 [BioGRID]:{'、'.join(str(i) for i in info['biogrid_interactions'][:3])}")
                if info.get("civic_variants"): parts.append(f"癌症变异 [CIViC]:{'、'.join(str(v) for v in info['civic_variants'][:2])}")
                if info.get("alliance_homologs"): parts.append(f"同源基因 [Alliance]:{'、'.join(str(h) for h in info['alliance_homologs'][:2])}")
                if info.get("ot_diseases"): parts.append(f"疾病关联 [Open Targets]:{'、'.join(str(d) for d in info['ot_diseases'][:3])}")
                if info.get("clinvar_variants"): parts.append(f"临床变异 [ClinVar]:{'、'.join(str(v) for v in info['clinvar_variants'][:2])}")
                if info.get("reactome_pathways"): parts.append(f"通路 [Reactome]:{'、'.join(str(p) for p in info['reactome_pathways'][:3])}")
                if info.get("drug_targets"): parts.append(f"药物靶标 [ChEMBL]:{'、'.join(str(d) for d in info['drug_targets'][:2])}")
                if info.get("interactions"): parts.append(f"互作 [STRING]:{'、'.join(str(i) for i in info['interactions'][:3])}")
                if info.get("regulators"): parts.append(f"调控TF [TRRUST]:{'、'.join(str(r) for r in info['regulators'][:3])}")
                if info.get("metabolites"): parts.append(f"代谢物 [HMDB]:{'、'.join(m.get('name','') if isinstance(m,dict) else str(m) for m in info['metabolites'][:3])}")
                if info.get("gut_microbes"): parts.append(f"肠道菌 [GUTMGENE]:{'、'.join(str(m) for m in info['gut_microbes'][:2])}")
                if parts:
                    lines.append(f"  [{g}] " + " | ".join(parts))
            for _pub_idx, a in enumerate(kb.get("pubmed", [])[:3], 1):
                pmid = a.get("pmid", "")
                lines.append(f"  [{_pub_idx}] [PubMed:{pmid}] {a.get('title','')[:80]}")
            for _epmc_idx, a in enumerate(kb.get("europepmc", [])[:2], 1):
                pmid = a.get("pmid", "")
                lines.append(f"  [{_epmc_idx}] [EuropePMC:{pmid}] {a.get('title','')[:80]}")

        lines.append("\n=== 各疾病表型 Top15 高表达基因 ===")
        for grp, genes in grp_map.items():
            kb = grp_knowledge.get(grp, {"genes": {}, "pubmed": [], "europepmc": []})
            lines.append(f"\n[{grp}] Top15: {'、'.join(genes)}")
            for g in genes[:8]:
                info = kb["genes"].get(g, {})
                parts = []
                if info.get("function"): parts.append(f"功能 [UniProt]:{info['function'][:120]}")
        ...
        lines.append(f"\n=== 总体高频 Top15 基因 ===")
        lines.append(", ".join(overall_top))

        context_str = "\n".join(lines)
        # Merge all pubmed/europepmc/genes across all groups for synthesizer
        all_pubmed = []
        all_europepmc = []
        all_gene_info = {}
        seen_pmids_comp = set()
        ...
        if self._vector_store is not None and self._vector_store.count() > 0:
            try:
                rag_context = self._vector_store.retrieve_context(message, n_results=12)
                if rag_context:
        ...
        import pandas as pd
        all_genes_comp = []
        for _genes in list(ct_map.values()) + list(grp_map.values()):
            all_genes_comp.extend(_genes)
        all_genes_comp = list(dict.fromkeys(all_genes_comp))[:40]
        fake_results_comp = {
            "deg": {"results": pd.DataFrame({"names": all_genes_comp}), "params": {}},
            "plots": [],
            "matrix_context": {
                "cell_type_focus": "all cell types and disease phenotypes",
                "genes_queried": all_genes_comp,
                "top_genes_per_celltype": ct_map,
                "top_genes_per_group": grp_map,
                # context_str intentionally omitted: raw per-gene API dump causes
