...
from e2sc.llm import RETRIEVER_PROMPT
    ...
    def retrieve(self, question: str, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.vector_store:
            try:
                ...
                logger.info(f"✓ Vector DB: Found {len(similar_cases)} similar cases")
            except Exception as e:
    ...
    def _retrieve_gene_info(self, gene: str) -> Dict[str, Any]:
        ...
        try:
            with self.string_db as db:
                ...
                if interactions:
                    logger.debug(f"✓ STRING: {len(interactions)} interactions for {gene}")
        except Exception as e:
            logger.debug(f"✗ STRING query failed for {gene}: {e}")
        ...
        try:
            with self.hmdb_db as db:
                ...
                if metabolites:
                    logger.debug(f"✓ HMDB: {len(metabolites)} metabolites for {gene}")
        except Exception as e:
            logger.debug(f"✗ HMDB query failed for {gene}: {e}")
        ...
        try:
            with self.trrust_db as db:
                ...
                if regulators:
                    logger.debug(f"✓ TRRUST: {len(regulators)} regulators for {gene}")
        except Exception as e:
            logger.debug(f"✗ TRRUST regulator query failed for {gene}: {e}")
        ...
        try:
            with self.trrust_db as db:
                ...
                if targets:
                    logger.debug(f"✓ TRRUST: {len(targets)} targets for {gene}")
        except Exception as e:
            logger.debug(f"✗ TRRUST target query failed for {gene}: {e}")
        ...
        try:
            with self.gutmgene_db as db:
                ...
                if microbes:
                    logger.debug(f"✓ GUTMGENE: {len(microbes)} microbes for {gene}")
        except Exception as e:
            logger.debug(f"✗ GUTMGENE query failed for {gene}: {e}")
        ...
        try:
            ...
            if uniprot_data and "results" in uniprot_data and len(uniprot_data["results"]) > 0:
                ...
                logger.debug(f"✓ UniProt: retrieved for {gene}")
        except Exception as e:
            logger.debug(f"✗ UniProt query failed for {gene}: {e}")
        ...
        try:
            ...
            if mg and mg.get("status") == "success":
                ...
                logger.debug(f"✓ MyGene: retrieved for {gene}")
        except Exception as e:
            logger.debug(f"✗ MyGene query failed for {gene}: {e}")
        ...
        try:
            ...
            if ens and ens.get("status") == "success":
                ...
                logger.debug(f"✓ Ensembl: retrieved for {gene}")
        except Exception as e:
            logger.debug(f"✗ Ensembl query failed for {gene}: {e}")
        ...
        try:
            ...
            if go and go.get("status") == "success" and go.get("total_annotations", 0) > 0:
                ...
                logger.debug(f"✓ QuickGO: {len(info['go_terms'])} GO terms for {gene}")
        except Exception as e:
            logger.debug(f"✗ QuickGO query failed for {gene}: {e}")
        ...
        try:
            ...
            if targets:
                ...
                logger.debug(f"✓ ChEMBL: {len(targets)} targets for {gene}")
        except Exception as e:
            logger.debug(f"✗ ChEMBL query failed for {gene}: {e}")
        ...
        try:
            ...
            if gtex and gtex.get("status") == "success" and gtex.get("total", 0) > 0:
                info["gtex"] = {"total_tissues": gtex.get("total"),
                                 "top_tissues": [{"tissue": r.get("tissue"), "median": r.get("median_expression")}
                                                 for r in gtex.get("records", [])[:5]]}
                logger.debug(f"✓ GTEx: {gtex.get('total')} tissues for {gene}")
        except Exception as e:
            logger.debug(f"✗ GTEx query failed for {gene}: {e}")
        ...
        try:
            ...
            if hb and hb.get("status") == "success" and hb.get("total", 0) > 0:
                ...
                logger.debug(f"✓ HumanBase: {hb.get('total')} network genes for {gene}")
        except Exception as e:
            logger.debug(f"✗ HumanBase query failed for {gene}: {e}")
        ...
        try:
            ...
            if gw and gw.get("status") == "success" and gw.get("total", 0) > 0:
                ...
                logger.debug(f"✓ GWAS: {gw.get('total')} associations for {gene}")
        except Exception as e:
            logger.debug(f"✗ GWAS query failed for {gene}: {e}")
        ...
        try:
            ...
            if bg and bg.get("status") == "success" and bg.get("total", 0) > 0:
                ...
                info["biogrid"] = [{"partner": i.get("gene_b") if i.get("gene_a") == gene else i.get("gene_a"),
                                     "evidence": i.get("evidence_type", "")} for i in ints[:8]]
                logger.debug(f"✓ BioGRID: {bg.get('total')} interactions for {gene}")
        except Exception as e:
            logger.debug(f"✗ BioGRID query failed for {gene}: {e}")
        ...
        try:
            ...
            if pm and pm.get("status") == "success" and pm.get("total_count", 0) > 0:
                ...
                logger.debug(f"✓ PubMed: {pm.get('total_count')} articles for {gene}")
        except Exception as e:
            logger.debug(f"✗ PubMed query failed for {gene}: {e}")
        ...
        try:
            ...
            if ep and ep.get("status") == "success" and ep.get("total_hits", 0) > 0:
                ...
                logger.debug(f"✓ EuropePMC: {ep.get('total_hits')} articles for {gene}")
        except Exception as e:
            logger.debug(f"✗ EuropePMC query failed for {gene}: {e}")
        ...
        try:
            ...
            if cv and cv.get("status") == "success" and cv.get("total", 0) > 0:
                ...
                logger.debug(f"✓ CIViC: {cv.get('total')} variants for {gene}")
        except Exception as e:
        ...
        try:
            ...
            if al and al.get("status") == "success" and al.get("total", 0) > 0:
                ...
                logger.debug(f"✓ Alliance: {al.get('total')} homologs for {gene}")
        except Exception as e:
            logger.debug(f"✗ Alliance query failed for {gene}: {e}")
