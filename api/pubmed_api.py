...
class PubMed_API:
    ...
    def __init__(self, email: str = None, api_key: str = None):
        ...
        self.email = str(email or os.environ.get("NCBI_EMAIL") or "e2seq@example.com").strip()
        self.api_key = str(api_key or os.environ.get("NCBI_API_KEY") or "").strip()

    def _request(self, url: str, params: Dict[str, Any]) -> requests.Response:
        """Send a rate-limited, retrying NCBI request shared by all clients."""
        ...
        for attempt in range(cls._max_attempts):
            with cls._request_lock:
                wait = cls._min_interval - (time.monotonic() - cls._last_request_at)
                if wait > 0:
                    time.sleep(wait)
            ...
            if response is not None:
                last_error = requests.HTTPError(
                    f"NCBI HTTP {response.status_code}", response=response
                )
                ...
                delay = 0.6 * (2 ** attempt)
            time.sleep(min(15.0, max(0.35, delay)))
    ...
    def get_article_details(self, pmid_list: List[str]) -> Dict[str, Any]:
        ...
        try:
            response = self._request(fetch_url, params)
            response.raise_for_status()
    ...
    def get_article_summary(self, pmid_list: List[str]) -> Dict[str, Any]:
        ...
        try:
            response = self._request(summary_url, params)
            response.raise_for_status()
            data = response.json()
